...
#print res
resource_version = get_res_version('test1')
def creat_prometheusrule_object():
    metadata=client.V1ObjectMeta(labels={"prometheus": "k8s","role":"alert-rules"},name='test1',namespace="monitoring",resource_version=resource_version)
    ...
    summary = "GJInstance {{ $labels.instance }} down"
    expr = "up{" + "instance!~" +' " ' + '.*8081"' + "}==0"
#    print expr
    rules=client.V1PrometheusRuleRule(alert="InstanceDown",expr=expr,ifor='15m',labels={"severity": "k8s_page","summary":summary},annotations={"test":"gjpei"})
...
def list_prometheusrule():
    ...
    #print body
    try:
        res = v1.patch_namespaced_prometheusrule(namespace='monitoring',name='test1',body=body)
        return 0,res
